...
export default function SubscriptionCard({ sub, detected, onGuide, onCancel, onHide, onEditPrice, isGuiding }: {
  ...
  return (
    <li className="grid grid-cols-[auto_1fr] sm:flex sm:items-center sm:justify-between card rounded-2xl p-4 sm:p-5 gap-3">
      <div className="flex items-center gap-3">
        ...
        <div className="font-medium flex items-center gap-2 leading-tight">
          <span className="text-base sm:text-sm">{sub.name}</span>
          {detected && (
      ...
      <div className="flex gap-2 items-center col-span-2 sm:col-span-1 mt-3 sm:mt-0">
        ...
        <button onClick={onHide} className="rounded-lg w-9 h-9 text-xs sm:text-xs border border-app hover:bg-[color:var(--surface)] tap inline-flex items-center justify-center" aria-label="Hide">✖</button>
      </div>
    </li>
  );
}
